  ...
  tron2_manipulation__msg__arm_status__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tron2_manipulation__msg__arm_status * data =
      (tron2_manipulation__msg__arm_status *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tron2_manipulation__msg__arm_status__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          tron2_manipulation__msg__arm_status__fini(&data[i]);
        }
        free(data);
        return false;
      }
