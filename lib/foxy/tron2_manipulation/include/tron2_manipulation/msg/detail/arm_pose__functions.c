  ...
  tron2_manipulation__msg__arm_pose__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tron2_manipulation__msg__arm_pose * data =
      (tron2_manipulation__msg__arm_pose *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tron2_manipulation__msg__arm_pose__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          tron2_manipulation__msg__arm_pose__fini(&data[i]);
        }
        free(data);
        return false;
      }
